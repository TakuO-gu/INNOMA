...
全テンプレートファイルを実際の自治体ウェブサイト（横浜市、大阪市、さいたま市等）と比較検証し、
欠けている情報の追加、定数の変数化、制度改正への対応を実施。

### tenkyo.json（転居届）の修正

**修正内容:**
- 印鑑（自治体により必要な場合がある）を必要書類に追加
- 後期高齢者医療資格確認書を必要書類に追加
- 介護保険被保険者証を必要書類に追加
- 在留カード（外国籍の方）を必要書類に追加
- 必要書類が自治体により異なることを示すcalloutを追加

**対象ファイル:**
- `apps/web/data/artifacts/_templates/services/registration/tenkyo.json`
- `apps/web/data/artifacts/sample/services/registration/tenkyo.json`

### kokuho.json（国民健康保険）の修正

**修正内容:**
- 保険料上限額（医療分67万円、後期支援分26万円、介護分17万円、合計110万円）を変数化
  - `{{kokuho_limit_medical}}`, `{{kokuho_limit_support}}`, `{{kokuho_limit_care}}`, `{{kokuho_limit_total}}`
- 保険料上限額が毎年度改定されることを示すcalloutを追加
- sample版では変数展開後の値を維持しつつ、calloutを追加

**対象ファイル:**
- `apps/web/data/artifacts/_templates/services/health/kokuho.json`
- `apps/web/data/artifacts/sample/services/health/kokuho.json`

### jidou-teate.json（児童手当）の修正 - 令和6年10月改正対応

**背景:**
令和6年10月から児童手当制度が大幅改正。旧テンプレートは改正前の情報のまま。

**旧制度:**
- 対象: 中学校修了前（15歳年度末まで）
- 所得制限あり（超過で特例給付5,000円、上限超で不支給）
- 第3子加算: 15,000円（3歳〜小学生のみ）
- 支給回数: 年3回（2月、6月、10月）
...
**修正内容:**
- Summaryを「中学校修了前」→「高校生年代まで」に変更
- 制度改正を知らせるNotificationBannerを追加
- 対象者説明を更新（所得制限なし）
- 支給額テーブルを新制度に対応
...
- 所得制限・所得上限テーブルを削除
- 所得制限撤廃を知らせるcalloutを追加
- 第3子カウントの説明を22歳年度末まで拡大に対応
- 支給日を年6回（偶数月）に変更
- 支給スケジュールテーブルを6回に拡張
- search情報を新制度に対応

**対象ファイル:**
- `apps/web/data/artifacts/_templates/services/childcare/jidou-teate.json`
- `apps/web/data/artifacts/sample/services/childcare/jidou-teate.json`

### TEMPLATE_VARIABLES.mdの更新
