...
import artifact from "@/data/artifacts/sample/services/childcare.json";
...
export default function ChildcareLandingPage() {
  ...
  // TopTaskブロックを取得
  const topTaskBlock = artifact.blocks.find(
    (b) => b.type === "TopTask"
  ) as { type: "TopTask"; props: { label: string; href: string; description?: string } } | undefined;
      ...
      <main className="flex-1">
        ...
        {topTaskBlock && (
          <section className="max-w-7xl mx-auto px-4 py-6">
            <NextLink
              href={topTaskBlock.props.href}
              className="block p-6 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
              <div className="text-xl font-bold mb-1">{topTaskBlock.props.label}</div>
              {topTaskBlock.props.description && (
                <div className="text-blue-100">{topTaskBlock.props.description}</div>
              )}
            </NextLink>
          </section>
        )}
...
function convertToInternalLink(href: string): string {
  ...
  if (href.startsWith("https://www.sample-city.lg.jp/services/childcare/nursery")) {
    return "/municipalities/sample/services/childcare/nursery";
  }
  // 他のURLはそのまま返す
  return href;
}
